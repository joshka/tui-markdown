...
//! That made it impossible for downstream crates to provide their own look-and-feel without forking
//! the crate.  This file introduces the [`StyleSheet`] trait: a minimal interface that supplies the
...
//! [`DefaultStyleSheet`] which matches the old behaviour and is used by default.  Projects that
//! want to theme the output can implement the trait for their own type and pass an instance via
//! [`crate::Options`].
...
///
/// Kept as a separate type so crates can `derive(Clone)` on their own theme and still fall back to
/// this one through [`crate::Options::default`].
